...
const team: TeamMember[] = [
  {
    ...
    bioZh: "Suddam Al-Salem是一位专注于钢结构、木结构和混凝土建筑的结构工程师。他曾在结构工程领域最具盛名的公司之一NCK积累了数年经验，参与了不同规模与复杂程度的项目。作为项目负责人，他在设计、工程项目管理及施工现场检查方面积累了全面的专业知识，能够掌控项目的一切技术与组织层面。",
  },
  {
    ...
    bioZh: "Muhannad在房地产开发领域拥有逾十年经验，在魁北克各地大型项目的规划、结构搭建与执行中发挥了关键作用。他在建筑与施工管理方面的背景，加上八年总承包商经验，使他对整个开发生命周期有着全面的理解。自2011年起，他与多个市政机构紧密合作，主导用地规划变更流程，争取项目审批，并与投资者携手构建财务可行、高影响力的开发项目。",
  },
];
        ...
        <FadeIn>
          <div className="mb-16 max-w-2xl">
            <p className="text-sm font-medium uppercase tracking-[0.15em] text-[#121212]/50">
              {isFr ? "Direction" : isZh ? "领导团队" : "Leadership"}
            </p>
            <h2
              className="mt-4 mb-8 font-medium text-[clamp(2rem,4vw,3rem)] leading-tight text-[#121212]"
            >
              {isFr ? "L'équipe de direction" : isZh ? "我们的团队" : "Executive team"}
            </h2>
          </div>
        </FadeIn>
              ...
              <article className="group">
                ...
                <div className="mt-8">
                  ...
                  <p className="mt-1 text-sm font-medium text-[#121212]/60">
                    {isFr ? member.roleFr : isZh ? member.roleZh : member.roleEn}
                  </p>
                  <a
                    href={`mailto:${member.email}`}
                    className="mt-2 inline-block text-sm text-[#0A5592] underline transition-colors duration-300 hover:text-[#121212]"
                  >
                    {member.email}
                  </a>
                  <p className="mt-4 text-sm leading-loose text-[#121212]/70">
                    {isFr ? member.bioFr : isZh ? member.bioZh : member.bioEn}
                  </p>
                </div>
              </article>
            </FadeIn>
